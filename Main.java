public class Main {
    ...
    static class MoveBlueprint {
        String name, category, effect;
        int power, type, accuracy;

        MoveBlueprint(String name, int power, int type, int accuracy, String category, String effect) {
            this.name     = name;
            this.power    = power;
            this.type     = type;
    ...
    static class Template {
        ...
        Template(String name, int type, int hp, int attack, int spAtk,
                 int defence, int spDef, int speed, MoveBlueprint[] moves) {
    ...
    static class Game {
        ...
        static Game get()  { if (instance == null) instance = new Game(); return instance; }
    ...
    static class CreatureData {
        ...
        private static MoveBlueprint mv(String name, int power, int type,
                                        int accuracy, String cat, String eff) {
        ...
        static final Template[] TEMPLATES = {
            ...
            /* 0 — Emberfox | Fire */
            new Template("Emberfox", Constants.T_FIRE, 480, 90, 85, 60, 65, 95, new MoveBlueprint[]{
                mv("Ember",        40,  Constants.T_FIRE,   100, "special",  "burn"),
                mv("Incinerate",   60,  Constants.T_FIRE,    90, "special",  null),
            ...
            /* 3 — Zapwing | Electric */
            new Template("Zapwing", Constants.T_ELECTRIC, 470, 80, 95, 60, 70, 115, new MoveBlueprint[]{
            ...
            /* 4 — Mindweave | Psychic */
            new Template("Mindweave", Constants.T_PSYCHIC, 490, 65, 110, 55, 100, 105, new MoveBlueprint[]{
            ...
            /* 5 — Grimclaw | Dark */
            new Template("Grimclaw", Constants.T_DARK, 510, 95, 75, 75, 70, 90, new MoveBlueprint[]{
                mv("Bite",         60, Constants.T_DARK,   100, "physical", null),
            ...
            /* 6 — Thedude | Normal */
            new Template("Thedude", Constants.T_NORMAL, 480, 80, 70, 70, 70, 80, new MoveBlueprint[]{
            ...
            /* 7 — Rockruff | Rock */
            new Template("Rockruff", Constants.T_ROCK, 580, 75, 55, 95, 85, 75, new MoveBlueprint[]{
                mv("Bite",            60,  Constants.T_DARK,     100, "physical", null),
            ...
            /* 8 — Elmacho | Fighting */
            new Template("Elmacho", Constants.T_FIGHTING, 500, 110, 70, 60, 60, 85, new MoveBlueprint[]{
    ...
    static class FighterFactory {
        static Fighter create(int pid, int[] moveIds) {
            ...
            for (int i = 0; i < Constants.MOVES_BATTLE; i++) {
                MoveBlueprint mb = t.moves[moveIds[i]];
    ...
    static class BattleEngine {
        ...
        // ── Damage calc ── returns: -2 no pp | -1 miss | 0 immune | >0 damage
        static int calcDamage(Fighter atk, Fighter def, int mi) {
            ...
            // FIX: was 15/10 (integer = 1), now correct
            if (m.type == atk.type) dmg = dmg * 15 / 10;
        ...
        // ── Execute one attack — returns true if defender faints
        static boolean doAttack(Fighter atk, Fighter def, int mi) {
            ...
            if (dmg == -2) { appendLog("[NO PP] " + atk.name + " can't use " + m.name + "!\n"); return false; }
            if (dmg == -1) { appendLog("[MISS] " + atk.name + " used " + m.name + "... Missed!\n"); return false; }
            if (dmg ==  0) { appendLog("[IMMUNE] " + def.name + " is immune!\n"); return false; }
            ...
            appendLog("[ATK] " + atk.name + " -> " + def.name + " : " + m.name + " " + dmg + " dmg!" + stab + eff + "\n");
            ...
            if (def.hp == 0) { appendLog("[KO] " + def.name + " fainted!\n"); return true; }
            return false;
        }

        // ── Struggle — used when all PP is empty
        static boolean allPpEmpty(Fighter f) {
            for (Move m : f.moves) if (m.PP > 0) return false;
            return true;
        }

        static boolean doStruggle(Fighter atk, Fighter def) {
            int dmg    = Math.max(1, 50 * atk.attack / (def.defence * 2));
            int recoil = Math.max(1, dmg / 4);
            def.hp = Math.max(0, def.hp - dmg);
            atk.hp = Math.max(0, atk.hp - recoil);
            appendLog("[STRUGGLE] " + atk.name + " used Struggle! (" + dmg + " dmg)\n");
            appendLog("[RECOIL] "   + atk.name + " took " + recoil + " recoil damage!\n");
            if (def.hp == 0) { appendLog("[KO] " + def.name + " fainted!\n"); return true; }
            return false;
        }

        // ── End of turn status damage — returns true if fighter faints
        ...
        static int EnemyPickMove() {
            ...
            int     cnt   = 0;
            for (int i = 0; i < Constants.MOVES_BATTLE; i++)
                if (ef.moves[i].PP > 0) avail[cnt++] = i;
        ...
        static void processRound(int playerMi) {
            ...
            boolean playerFirst = pf.speed > ef.speed ||
                                  (pf.speed == ef.speed && Math.random() < 0.5);
            ...
            if (playerFirst) {
                ...
                if (allPpEmpty(pf)) {
                    ...
                    if (pf.hp == 0) {
                        ...
                        g.forceSwitch = true; return;
                    }
                } else if (pf.moves[playerMi].PP <= 0) {
                ...
                em = EnemyPickMove();
                if (em < 0) {
                    // FIX: enemy struggles instead of skipping
                    if (doStruggle(ef, pf)) {
                        if (countAlive(g.player) == 0) { g.result = -1; g.phase = Phase.OVER; return; }
                        g.forceSwitch = true; return;
                    }
                    ...
                    if (doAttack(ef, pf, em)) {
                        // FIX: was g.result = 1 (wrong), now -1
                        if (countAlive(g.player) == 0) { g.result = -1; g.phase = Phase.OVER; return; }
                        g.forceSwitch = true;
                    }
                }
                ...
                if (em < 0) {
                    ...
                    if (ef.hp == 0) {
                        if (countAlive(g.enemy) == 0) { g.result = 1; g.phase = Phase.OVER; return; }
                        enemyAutoSwitch(); ef = g.enemy.slots[g.enemy.active];
                    }
                } else {
                    if (doAttack(ef, pf, em)) {
                        if (countAlive(g.player) == 0) { g.result = -1; g.phase = Phase.OVER; return; }
